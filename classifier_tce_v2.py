...
# E4
model_1=RandomForestClassifier(n_estimators=10,warm_start=True,criterion='gini',max_depth=30, random_state=3)
model_2 = KNeighborsClassifier(n_neighbors=25)
model_3 = LogisticRegression(random_state=3, solver='lbfgs',multi_class='multinomial',max_iter =1000)
...
for i in range(10):
    ...
    new_list_features=select_k_best(train_x,train_y,k)
    ...
    train_x_new=train_x[new_list_features]
    valid_x_new=valid_x[new_list_features]
    fitted=fitted.fit(train_x_new,train_y)
    # fitted=fitted.set_params(n_estimators=50)
    fitted.n_estimators += 50
    fitted=fitted.fit(train_x_new,train_y)
    # predicted_1=fit.predict(valid_x_new)
    # new_mcc=matthews_corrcoef(valid_y, predicted_1)
    # if new_mcc>=base_mcc:
    #     fitted=fit
    #     base_mcc=new_mcc
    #     mcc_scores.append(base_mcc)
    #     list_features=new_list_features
    



list_features=select_k_best(X_train_new,label_2_train,k)
...
predicted_1_test=fitted.predict(X_test_new)
